...
이 문서는 현재 프로젝트에서 Kubernetes가 맡는 역할과 실제 설정값을 정리한 것이다.

목표는 `university` namespace에 MariaDB, Redis, Department API, University Vue 애플리케이션을 배포하고, Ingress와 cert-manager를 통해 외부 HTTP/HTTPS 접근까지 구성하는 것이다.

> 참고: `06_Volume`의 `hostPath` 경로는 환경별 Docker Desktop 파일 공유 경로에 맞게 수정해야 한다. 자세한 내용은 아래 “hostPath 경로 수정” 섹션을 참고한다.
...
## 2) 06_Volume 적용 (Volume + MariaDB PV/PVC)
...
> chart repository와 values 파일을 기준으로 Helm 설치를 진행한다.
...
현재 구성에서는 NGINX 기반 Ingress Controller를 사용한다.
...
현재 구성에서는 self-signed ClusterIssuer를 사용해서
...
Docker Desktop Kubernetes라면, 보통 Mac에서는 아래처럼 “Users 경로”로 바꿔야 한다. 정확한 값은 로컬 환경마다 다를 수 있다.
...
### 12-9) university 앱의 대략적인 메모리 감각
